...
@section('page-subtitle', 'Buat aturan RBS baru untuk analisis pemupukan')
...
<div class="max-w-4xl mx-auto space-y-5">
    ...
    <form method="POST" action="{{ route('rule-base.store') }}" class="space-y-5">
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="kondisi_warna_daun" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['Hijau Normal','Hijau Pucat','Kuning Merata','Kuning Tepi','Kuning Antar Tulang','Oranye/Kemerahan','Coklat Ujung','Bercak Nekrotik'] as $opt)
                            <option value="{{ $opt }}" {{ old('kondisi_warna_daun') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="kondisi_ph_min" value="{{ old('kondisi_ph_min') }}" step="0.1" min="3" max="8" placeholder="cth: 3.0"
                    ...
                    <p class="text-[10px] text-slate-400 mt-0.5">Batas bawah pH (kosong = tidak dicek)</p>
                </div>

                {{-- pH Max --}}
                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1.5">pH Maksimum</label>
                    <input type="number" name="kondisi_ph_max" value="{{ old('kondisi_ph_max') }}" step="0.1" min="3" max="8" placeholder="cth: 4.5"
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi_kelembaban" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['Sangat Kering','Kering','Normal','Lembab','Sangat Lembab'] as $opt)
                            <option value="{{ $opt }}" {{ old('kondisi_kelembaban') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi_musim" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['Musim Hujan','Musim Kemarau','Peralihan'] as $opt)
                            <option value="{{ $opt }}" {{ old('kondisi_musim') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi_defisiensi" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['N','P','K','Mg','B','Fe','Zn'] as $opt)
                            <option value="{{ $opt }}" {{ old('kondisi_defisiensi') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi_kategori_umur" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['Belum Menghasilkan','Remaja','Menghasilkan Muda','Menghasilkan Tua','Tua Renta'] as $opt)
                            <option value="{{ $opt }}" {{ old('kondisi_kategori_umur') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="ada_serangan_hama" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="null" {{ old('ada_serangan_hama', 'null') == 'null' ? 'selected' : '' }}>— Tidak dicek —</option>
                        <option value="1" {{ old('ada_serangan_hama') == '1' ? 'selected' : '' }}>Harus ada hama</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi_curah_hujan_kategori" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        <option value="{{ $ch }}" {{ old('kondisi_curah_hujan_kategori') == $ch ? 'selected' : '' }}>{{ $ch }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="ada_gulma_dominan" class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="null" {{ old('ada_gulma_dominan', 'null') == 'null' ? 'selected' : '' }}>— Abaikan —</option>
                        <option value="1" {{ old('ada_gulma_dominan') == '1' ? 'selected' : '' }}>Ya</option>
                        <option value="0" {{ old('ada_gulma_dominan') === '0' ? 'selected' : '' }}>Tidak</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="indikasi_masalah" value="{{ old('indikasi_masalah') }}" required placeholder="cth: Defisiensi Nitrogen — Klorosis Umum"
                ...
                <div>
                    ...
                    <input type="text" name="jenis_pupuk_utama" value="{{ old('jenis_pupuk_utama') }}" required placeholder="cth: Urea (46% N)"
                ...
                <div>
                    ...
                    <input type="text" name="jenis_pupuk_pendukung" value="{{ old('jenis_pupuk_pendukung') }}" placeholder="cth: ZA (21% N)"
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-xs font-medium text-slate-700 mb-1.5">Dosis Anjuran <span class="text-red-400">*</span></label>
                    <input type="text" name="dosis_anjuran" value="{{ old('dosis_anjuran') }}" required placeholder="cth: 1.5–2.0 kg Urea/pokok, 2–3 kali/tahun"
                ...
                <div>
                    ...
                    <input type="text" name="metode_aplikasi" value="{{ old('metode_aplikasi') }}" placeholder="cth: Ditabur melingkar 1–2 m dari pangkal batang"
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                </div>

                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1.5">Waktu Aplikasi</label>
                    <input type="text" name="waktu_aplikasi" value="{{ old('waktu_aplikasi') }}" placeholder="cth: Awal musim hujan (Maret–April)"
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-xs font-medium text-slate-700 mb-1.5">Saran Tindakan <span class="text-red-400">*</span></label>
                    <textarea name="saran_tindakan" rows="3" required placeholder="Penjelasan detail tindakan yang perlu dilakukan..."
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 resize-none @error('saran_tindakan') border-red-400 @enderror">{{ old('saran_tindakan') }}</textarea>
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="status_kebutuhan" required class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 @error('status_kebutuhan') border-red-400 @enderror">
                        ...
                        @foreach(['Darurat' => 'Darurat (Defisiensi Berat)', 'Segera' => 'Segera (Perlu Pupuk)', 'Normal' => 'Normal (Sehat)', 'Tunda' => 'Tunda (Tunda Pupuk)'] as $val => $label)
                            <option value="{{ $val }}" {{ old('status_kebutuhan') == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('status_kebutuhan') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs font-medium text-slate-700 mb-1.5">Prioritas (1–10) <span class="text-red-400">*</span></label>
                    <input type="number" name="prioritas" value="{{ old('prioritas', 5) }}" min="1" max="10" required
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 @error('prioritas') border-red-400 @enderror">
            ...
            <div class="mt-4">
                ...
                <textarea name="keterangan_rule" rows="2" placeholder="Catatan internal tentang rule ini (opsional)..."
                    class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 resize-none">{{ old('keterangan_rule') }}</textarea>
            </div>
        </div>
        ...
        <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3">
            ...
            <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm">Simpan Rule</button>
        </div>
    </form>
</div>
@endsection
