        ...
        <form method="POST" action="{{ route('blok-lahan.update', $blokLahan) }}" class="space-y-5" id="form-blok-lahan">
            ...
            <div>
                ...
                <div id="panel-draw" class="space-y-3">
                    <div id="draw-map"></div>
                    <p class="text-xs text-slate-500">Area <span class="text-amber-600 font-semibold">kuning</span> = lahan milik anggota lain. Gunakan tombol edit (pensil) untuk menggeser titik polygon.</p>
                </div>
...
drawMap.on(L.Draw.Event.CREATED, function(e) { drawnItems.clearLayers(); drawnItems.addLayer(e.layer); updateGeoJson(); });
drawMap.on(L.Draw.Event.EDITED, updateGeoJson);
drawMap.on(L.Draw.Event.DELETED, updateGeoJson);

function updateGeoJson() {
    var layers = drawnItems.getLayers();
    var val = layers.length > 0 ? JSON.stringify(layers[0].toGeoJSON().geometry) : '';
    document.getElementById('koordinat_geojson').value = val;
    document.getElementById('textarea_geojson').value = val;
}
