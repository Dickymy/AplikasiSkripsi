    ...
    <form action="{{ route('kondisi-lahan.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="blok_lahan_id"
                        class="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors @error('blok_lahan_id') border-red-400 @enderror"
                        required>
                        <option value="">— Pilih Blok —</option>
                        @foreach($bloks as $blok)
                            <option value="{{ $blok->id }}"
                                {{ (old('blok_lahan_id', $selectedBlokId) == $blok->id) ? 'selected' : '' }}>
                                {{ $blok->nama_blok }} — {{ $blok->nama_pemilik }}
                            </option>
                        @endforeach
                    </select>
                    @error('blok_lahan_id')
                ...
                <div>
                    ...
                    <select name="curah_hujan_kategori"
                        ...
                        @foreach(['Sangat Rendah','Rendah','Normal','Tinggi','Sangat Tinggi'] as $opt)
                            <option value="{{ $opt }}" {{ old('curah_hujan_kategori') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
            ...
            <div class="mb-5">
                ...
                <p class="mt-2 text-xs text-slate-400">N=Nitrogen, P=Fosfor, K=Kalium, Mg=Magnesium, B=Boron, Fe=Besi, Zn=Seng</p>
            </div>
